...
int main() {
	...
	class _long_long_number {                  // 64 бита (+/-)  Не забыть про деструктор !
	public:
		int high;                              // 32 бита (+/-)
		...
		bool ravno(const _long_long_number& number) {
			if ((high == number.high) && (low == number.low)) return true;
			else return false;
		};
		bool ravno(const int _high, const unsigned int _low) {
			if ((high == _high) && (low == _low)) return true;
			else return false;
		};
		bool bolshe(const _long_long_number& number) {
			if ((high > number.high) || (high == number.high) && (low > number.low)) return true;
			else return false;
		};
		bool bolshe(const int _high, const unsigned int _low) {
			if ((high > _high) || ((high == _high) && (low > _low))) return true;
			else return false;
		};
		bool menshe(const _long_long_number& number) {
			if ((high <= number.high) || (high == number.high) && (low < number.low)) return true;
			else return false;
		};
		bool menshe(const int _high, const unsigned int _low) {
			if ((high <= _high) || ((high == _high) && (low < _low))) return true;
			else return false;
		};
		void plus(_long_long_number& n1) {
			int H = high;
			unsigned int L = low;
			high += n1.high;
			low += n1.low;
		...
		void minus(_long_long_number& n1) {
			int H = high;
			unsigned int L = low;
			high -= n1.high;
			if (low < n1.low) {
		...
		void assign(_longlong_result number) {
			high = number >> 32;			   
			low = number - (number << 32);
		}
		void assign(int _high, unsigned int _low) {
			high = _high;			  
			low = _low;
		}
		void multiply(_long_long_number& number) {
		...
		void divide(_long_long_number& number) {
			_longlong_result result_1 = 0, result_2 = 0;
			result_1 = ((result_1 ^ high) << 32) ^ low;
			result_2 = ((result_2 ^ number.high) << 32) ^ number.low;
			result_1 /= result_2;
